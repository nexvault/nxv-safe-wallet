...
def __init__(_weth: address):
    WETH20 = _weth
    self.token = 0x0000000000000000000000000000000000000001
...
def exchange(i: uint256, j: uint256, dx: uint256, min_dy: uint256,
    ...
    """
    return self._exchange(msg.sender, msg.value, i, j, dx, min_dy, use_eth, receiver, empty(address), empty(bytes32))
...
def exchange_underlying(i: uint256, j: uint256, dx: uint256, min_dy: uint256,
    ...
    Exchange using ETH
    """
    return self._exchange(msg.sender, msg.value, i, j, dx, min_dy, True, receiver, empty(address), empty(bytes32))
...
def initialize(
    ...
    assert self.token == ZERO_ADDRESS  # dev: check that we call it from factory
